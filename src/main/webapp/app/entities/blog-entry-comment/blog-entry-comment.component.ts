import { Component, OnInit, OnDestroy } from '@angular/core';
import { HttpErrorResponse, HttpResponse } from '@angular/common/http';
import { Subscription } from 'rxjs';
import { JhiEventManager, JhiAlertService, JhiDataUtils } from 'ng-jhipster';

import { IBlogEntryComment } from 'app/shared/model/blog-entry-comment.model';
import { Principal } from 'app/core';
import { BlogEntryCommentService } from './blog-entry-comment.service';

@Component({
    selector: 'jhi-blog-entry-comment',
    templateUrl: './blog-entry-comment.component.html'
})
export class BlogEntryCommentComponent implements OnInit, OnDestroy {
    blogEntryComments: IBlogEntryComment[];
    currentAccount: any;
    eventSubscriber: Subscription;

    constructor(
        private blogEntryCommentService: BlogEntryCommentService,
        private jhiAlertService: JhiAlertService,
        private dataUtils: JhiDataUtils,
        private eventManager: JhiEventManager,
        private principal: Principal
    ) {}

    loadAll() {
        this.blogEntryCommentService.query().subscribe(
            (res: HttpResponse<IBlogEntryComment[]>) => {
                this.blogEntryComments = res.body;
            },
            (res: HttpErrorResponse) => this.onError(res.message)
        );
    }

    ngOnInit() {
        this.loadAll();
        this.principal.identity().then(account => {
            this.currentAccount = account;
        });
        this.registerChangeInBlogEntryComments();
    }

    ngOnDestroy() {
        this.eventManager.destroy(this.eventSubscriber);
    }

    trackId(index: number, item: IBlogEntryComment) {
        return item.id;
    }

    byteSize(field) {
        return this.dataUtils.byteSize(field);
    }

    openFile(contentType, field) {
        return this.dataUtils.openFile(contentType, field);
    }

    registerChangeInBlogEntryComments() {
        this.eventSubscriber = this.eventManager.subscribe('blogEntryCommentListModification', response => this.loadAll());
    }

    private onError(errorMessage: string) {
        this.jhiAlertService.error(errorMessage, null, null);
    }
}
